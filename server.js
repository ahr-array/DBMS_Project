...
app.post('/submitstock', checkAuthenticated, (req, res) => {
    ...
    let sql = `INSERT INTO stockdb (ItemID, ItemName, Category, Brand, Size, Amount, ImageURL, DATE_FORMAT(StockDate, 'YYYY/MM/DD') AS StockDate, StockTime, TDay, TMonth, TYear) VALUES ?`;
    let query = mysqlConnection.query(sql, [data], (err, rows, fields) => {
        if (!err) {
